...
extern uthread_struct_t *sched_find_best_uthread(kthread_runqueue_t *kthread_runq)
{
	...
	int ii;//added for balancing	
	kthread_context_t *iter_kthread, *current_kthread;
	kthread_runqueue_t *iter_runqueue;
	runqueue_t *iter_active_runqueue;
...
//	if(choice == 1)	
//	current_kthread = kthread_cpu_map[kthread_apic_id()];
	...
	if(!(runq->uthread_mask))
	{
        /*  	
		if(choice == 1){ 
		// No jobs in active. Search in other runqueues
		 
		gt_spin_unlock(&(kthread_runq->kthread_runqlock));
		
		for(ii=0;ii<4 && ii!=current_kthread->cpuid;ii++)	
		{
			
			iter_kthread = kthread_cpu_map[ii];
			iter_runqueue = &(iter_kthread->krunqueue);
			gt_spin_lock(&(iter_runqueue->kthread_runqlock));
			iter_active_runqueue = iter_runqueue->active_runq;
			
			
			if(!iter_active_runqueue->uthread_mask)//empty active q
			{ 
				gt_spin_unlock(&(iter_runqueue->kthread_runqlock));
				break;
			}
			else
			{
				//printf("LOAD BALANCING\n");
				// Find the highest priority bucket in other qs
				uprio = LOWEST_BIT_SET(iter_active_runqueue->uthread_mask);
				prioq = &(iter_active_runqueue->prio_array[uprio]);
				...
				__rem_from_runqueue(iter_active_runqueue, u_obj);
