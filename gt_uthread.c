...
extern void uthread_schedule(uthread_struct_t * (*kthread_best_sched_uthread)(kthread_runqueue_t *))
{
	...
	if((u_obj = kthread_runq->cur_uthread))
	{
		if(choice == 1)
		{
			gettimeofday(&(u_obj->preempted_time), NULL);
			timersub(&(u_obj->preempted_time), &(u_obj->scheduled_time), &(u_obj->executed_time)); 
			executed_time = u_obj->executed_time.tv_sec * 1000000L +					u_obj->executed_time.tv_usec;
			//Explicit mention of long to prevent overflow
                        exetime[u_obj->uthread_tid] = exetime[u_obj->uthread_tid] + executed_time; 
			u_obj->credits -= (executed_time/10000);
			//printf("\n*****%d*******\n ",u_obj->credits); 
		}
		...
		else
		{
			...
			if(choice == 1)//Credit Scheduler
			{
				if(u_obj->credits > 0)//Credits remaing
				{       
					add_to_runqueue(kthread_runq->active_runq, &(kthread_runq->kthread_runqlock), u_obj);				
				}
				else // Credits over => expired
				{
					u_obj->credits = u_obj->weight;
